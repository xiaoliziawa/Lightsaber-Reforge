...
public class ALData<T> extends FiskRegistryEntry<ALData<?>>
{
    ...
    public boolean hasPerms(LogicalSide side)
    {
    ...
    protected boolean validUpdate(Entity entity, T value)
    {
        ...
        {
            return !ItemStack.matches((ItemStack) get(entity), (ItemStack) value);
        }

        return !Objects.equals(value, get(entity));
    }
    ...
    private boolean setWithBypass(Entity entity, T value)
    {
        if (validUpdate(entity, value))
        {
            ...
            else if (entity instanceof Player player)
            {
                ...
                ALPlayerData.getData(player).putData(this, value);
                onValueChanged(entity, value);
            }
    ...
    public T get(Entity entity)
    {
        ...
        else if (entity instanceof Player player && ALPlayerData.hasData(player))
        {
            return ALPlayerData.getData(player).getData(this);
        }
    ...
    public boolean sync(Player player)
    {
        ...
        if (hasPerms(side) && legalUpdate(player))
        {
            ...
            {
                ALNetworkManager.sendToTrackingAndSelf(player, message);
            }
    ...
    public boolean set(Player player, T value)
    {
        return setWithoutNotify(player, value) && sync(player);
    }

    public boolean incr(Player player, T value)
    {
        return incrWithoutNotify(player, value) && sync(player);
    }

    public boolean clamp(Player player, T min, T max)
    {
        return clampWithoutNotify(player, min, max) && sync(player);
    }

    public static CompoundTag writeToNBT(CompoundTag nbt, Map<ALData<?>, Object> data)
    {
        CompoundTag dataTag = new CompoundTag();

        for (Map.Entry<ALData<?>, Object> entry : data.entrySet())
        {
            if (entry.getKey().shouldSaveNBT())
            {
                Tag valueTag = NBTHelper.writeToNBT(entry.getValue());

                if (valueTag != null)
                {
                    dataTag.put(entry.getKey().id, valueTag);
                }
            }
        }

        nbt.put("DataArray", dataTag);
    ...
    public static void init()
    {
        for (Field field : ALData.class.getFields())
        {
            if (ALData.class.isAssignableFrom(field.getType()))
            {
                ...
                {
                    Lightsabers.LOGGER.error("Failed to initialize data field {}", field.getName(), e);
                }
            }
        }
    }
