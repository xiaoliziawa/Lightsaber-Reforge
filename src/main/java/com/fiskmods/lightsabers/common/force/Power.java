...
public class Power implements Comparable<Power>, ISerializableObject<Power>
{
    ...
    {
        MutableComponent title = Component.translatable(getUnlocalizedName()).withStyle(forceSide.theme);
        MutableComponent description = title.copy();

        if (powerStats.useCost > 0)
        {
            String key = powerStats.powerType == PowerType.PER_USE
                    ? "forcepower.perUse"
    ...
    public Component getChatFormattedName()
    {
        Component title = getFormattedName();
        return Component.literal("[").append(title).append("]").withStyle(title.getStyle());
    }

    public int getActualXpCost(Player player)
    {
    ...
    public float getUseCost(Player player)
    {
    ...
    {
        NBTHelper.toBytes(buf, getName());
    }
    
    public static class Adapter implements ISaveAdapter<Power>
    {
        @Override
        public Power readFromNBT(Tag tag)
        {
        ...
        {
            return Power.getPowerFromName(NBTHelper.fromBytes(buf, String.class));
        }
    }
}
